...
function proxied(url){
  ...
  return 'https://images.weserv.nl/?url=' + url.replace(/^https?:\/\//,'') + '&w=600&h=600&fit=contain';
}
...
// Níveis de dados baseados em dataStatus (campo autoritativo no JSON).
// complete = perfil com stats + fights verificados
// review   = dados ainda não verificados manualmente
// basic    = apenas informações mínimas
function dataLevel(f){
  const ds = f.dataStatus;
  if(ds === 'complete') return 'complete';
  if(ds === 'basic')    return 'basic';
  return 'review'; // padrão: review
}

function hasStats(f){
  if(!f.stats) return false;
  return Object.values(f.stats).some(v => v !== null && v !== undefined && Number(v) > 0);
}
...
function updateCounter(){
  const all = visibleFighters();
  const total = all.length;
  const complete = all.filter(f => dataLevel(f) === 'complete').length;
  const review  = all.filter(f => dataLevel(f) === 'review').length;
  const basic   = all.filter(f => dataLevel(f) === 'basic').length;
  $('counter').textContent = `${total} lutadores · ${complete} completos · ${review} em revisão · ${basic} básicos`;
}
...
function levelLabel(f){
  const lv = dataLevel(f);
  if(lv === 'review') return '· <span style="color:#C9A84C">em revisão</span>';
  if(lv === 'basic')  return '· perfil básico';
  return '';
}
    ...
    box.innerHTML = res.map(f => `
      <div class="si" data-id="${safe(f.id)}">
        ...
        <div>
          ...
          <div class="si-div">${safe(f.div)} · ${safe(f.country)} ${levelLabel(f)}</div>
        </div>
      </div>`).join('');
...
function randomCompare(){
  // Inclui "review", exclui apenas "basic"
  const list = visibleFighters().filter(f => dataLevel(f) !== 'basic');
...
function qualityBadge(f){
  const lv = dataLevel(f);
  if(lv === 'review'){
    return `<div class="badge-quality" style="background:rgba(201,168,76,0.12);color:#C9A84C;border:1px solid rgba(201,168,76,0.25);border-radius:4px;font-size:.6rem;letter-spacing:2px;padding:3px 8px;margin-top:6px;text-transform:uppercase">⚠ dados em revisão · ${safe(f.dataUpdated || 'sem data')}</div>`;
  }
  if(lv === 'basic'){
    return `<div class="badge-quality basic">perfil básico · ${safe(f.dataUpdated || 'sem data')}</div>`;
  }
  return `<div class="badge-quality">perfil completo · ${safe(f.dataUpdated || 'sem data')}</div>`;
}

function card(f, cls, label){
  const r = f.record || {}, fi = f.fin || {};
  const showFin = fi.ko !== null || fi.sub !== null || fi.dec !== null;
  const showPhys = f.height || f.reach || f.age || f.stance || f.team;
  return `<div class="fc ${cls}">
    ...
    <div class="finfo">
      ...
      ${f.title && f.title !== '— (dados em revisão)' ? `<div style="font-size:.63rem;color:var(--gold);margin-top:4px">${safe(f.title)}</div>` : ''}
      ${qualityBadge(f)}
      <div class="rec">
...
function hist(f){
  const lv = dataLevel(f);
  const fights = f.fights || [];

  if(!fights.length){
    const msg = lv === 'review'
      ? 'Histórico ainda não verificado para este perfil.'
      : 'Histórico ainda não cadastrado neste perfil. Complete o campo <strong>fights</strong> no arquivo <strong>data/fighters.json</strong>.';
    return `<div class="hcard">
      <div class="hhdr">${safe(f.name)} · últimas lutas</div>
      <div class="empty-hist">${msg}</div>
    </div>`;
  }

  const rows = fights.slice(0,5).map(x => `
    <div class="frow">
...
function render(f1, f2){
  ...
  const showRadar = hasStats(f1) && hasStats(f2);
  const showStats = hasStats(f1) || hasStats(f2);
  ...
  const radarSection = showRadar ? `
    ...
    <div class="stitle">RADAR COMPARATIVO</div>
    <div style="text-align:center;padding:24px;color:rgba(255,255,255,0.3);font-size:.8rem;letter-spacing:2px;text-transform:uppercase">
      Radar indisponível — estatísticas não verificadas para um ou ambos os lutadores
    </div>`;
  ...
  const statsSection = showStats ? `
    ...
    <div class="stitle">ESTATÍSTICAS DETALHADAS</div>
    <div style="text-align:center;padding:24px;color:rgba(255,255,255,0.3);font-size:.8rem;letter-spacing:2px;text-transform:uppercase">
      Estatísticas não verificadas para estes lutadores
    </div>`;
